...
static float max_lin_velocity = 0.5; //0.5; // maximum linear velocity in m/s
static float max_lin_acceleration = .25; // maximum linear acceleration in m/s/s

static float max_lin_velocity_for_traj = 0.5; //0.5; // maximum linear velocity in m/s
static float max_lin_acceleration_for_traj = .3; // maximum linear acceleration in m/s/s

static float max_lin_velocity_for_return = 2;  //1; //.5; // maximum linear velocity in m/s
static float max_lin_acceleration_for_return = 0.8;    //0.5;    // .25; // maximum linear acceleration in m/s/s

static float max_lin_velocity_reduce_height = 2;  //1; //.5; // maximum linear velocity in m/s
static float max_lin_acceleration_reduce_height = 0.8;    //0.5;    // .25; // maximum linear acceleration in m/s/s
...
static float max_ang_velocity_for_traj = 1.5;    //1.0; // desired velocity in rad/s
static float max_ang_acceleration_for_traj = 1.0; //0.5; // desired acceleration in rad/s/s



#define TRAJ_MISSION_LOG_FILE  "/home/linaro/flight_path/traj_mission_log.csv"
...
/**********************************************************************************************************
 ************************************Add by zzg************************************************************
 ****goto_waypoint            limit the xy_diff,xy_vel(0.05, 0.01), yaw_diff,yay_vel(0.008, 0.02)**********
 ****goto_waypoint_with_gps   limit the xy_diff,xy_vel(   1,  0.3), yaw_diff,yay_vel(0.008, 0.02)**********
 ****goto_waypoint_for_circle limit the xy_diff,xy_vel( 0.1, 0.05), yaw_diff,yay_vel(0.015, 0.05)**********
***********************************************************************************************************/
...
int goto_waypoint_for_traj(FlatVars input_state, FlatVars goal_state, FlatVars last_commanded_world_velocity, bool stop_at_waypoint, FlatVars * cmd_out, uint8_t * reached_goal, float xyz_goal_deviation, bool yaw_only)
{
  // This function computes the desired velocity to reach a goal state (location+yaw) from a starting state while respecting dynamics and obeying a maximum velocity and acceleration
  // This function requires as input the previously specified velocity to ensure continuity in velocity.
  // All coordinates are in the world frame. This means that body commanded input velocities will need to be transformed to the world frame and outputs that should be in the body frame will need to be transformed.
  ...
  *reached_goal = 0b11111111;
  ...
  float last_vel_mag = sqrtf(last_commanded_world_velocity.x*last_commanded_world_velocity.x +last_commanded_world_velocity.y*last_commanded_world_velocity.y +last_commanded_world_velocity.z*last_commanded_world_velocity.z);
  float last_xy_vel_mag = sqrtf(last_commanded_world_velocity.x*last_commanded_world_velocity.x +last_commanded_world_velocity.y*last_commanded_world_velocity.y);
  ...
  // compute maximum possible velocity capable of stopping under maximum acceleration
  float max_vel_close_mag = sqrtf(2*max_lin_acceleration_for_traj*goal_vec_mag);

  // now compute the desired magnitude of velocity as the smaller of the stopping velocity and max allowed velocity
  float des_velocity = max_lin_velocity_for_traj;

  if(stop_at_waypoint == true)
  {
    MIN(des_velocity,max_vel_close_mag,max_lin_velocity_for_traj);
  }
  ...
  // compute applied acceleration to influence velocity in correct direction
  float accel_applied_x = unit_des_vel_change_x*max_lin_acceleration_for_traj;
  float accel_applied_y = unit_des_vel_change_y*max_lin_acceleration_for_traj;
  float accel_applied_z = unit_des_vel_change_z*max_lin_acceleration_for_traj;
  ...
  // bound current velocity by specified limit
  float current_vel_mag = sqrtf(current_cmd_world_velocity.x*current_cmd_world_velocity.x+current_cmd_world_velocity.y*current_cmd_world_velocity.y+current_cmd_world_velocity.z*current_cmd_world_velocity.z);
  if (current_vel_mag > max_lin_velocity_for_traj)
  {
    current_cmd_world_velocity.x = current_cmd_world_velocity.x/current_vel_mag*max_lin_velocity_for_traj;
    current_cmd_world_velocity.y = current_cmd_world_velocity.y/current_vel_mag*max_lin_velocity_for_traj;
    current_cmd_world_velocity.z = current_cmd_world_velocity.z/current_vel_mag*max_lin_velocity_for_traj;
  }
  ...
  //if ((last_vel_mag<xyz_last_vel_mag) || (stop_at_waypoint == false) || (yaw_only == true))
  if (((fabs(last_commanded_world_velocity.z)<0.3) && (last_xy_vel_mag < 0.1)) || (stop_at_waypoint == false) || (yaw_only == true))
  {
    *reached_goal=*reached_goal&0b11110001;
  }
  ...
  // determine max anglular velocity that supports stopping at given angle
  float max_vel_yaw_close_mag = sqrtf(2*max_ang_acceleration_for_traj*yaw_error_abs);

  // compute desired angular velocity to reach goal location
  float des_ang_velocity = max_ang_velocity_for_traj;
  MIN(des_ang_velocity,max_vel_yaw_close_mag,max_ang_velocity_for_traj);
  ...
  // compute yaw acceleration to use
  float accel_applied_yaw = yaw_acceleration_direction*max_ang_acceleration_for_traj;
  // increment commanded angular velocity by desired accleration*dt
  current_cmd_world_velocity.yaw = last_commanded_world_velocity.yaw + accel_applied_yaw*dt;

  // bound maximum yaw rate
  current_cmd_world_velocity.yaw = constrain_min_max(current_cmd_world_velocity.yaw,-max_ang_velocity_for_traj,max_ang_velocity_for_traj);

  // if yaw error within threshhold of zero, set set yaw position flag to 0
  //if((yaw_error_abs<.008) || (stop_at_waypoint == false))
  if((yaw_error_abs<.08) || (stop_at_waypoint == false))
  {
    *reached_goal=*reached_goal&0b11101111;
  }
  // if yaw velocity within threshhold of zero, set set yaw velocity flag to 0
  ...
  memcpy(&previous_desired_velocity,&current_cmd_world_velocity,sizeof(FlatVars));
    ...
    FILE *fp;
    if ((fp = fopen(TRAJ_MISSION_LOG_FILE, "a+")) != NULL)
    {
        char debug_msg[128];
        memset(debug_msg, 0, 128);
        sprintf(debug_msg, "%f,%f,%f,%f,%f,%f,%f\n",
                    goal_closest_point_mag,
                    goal_state.x-goal_closest_point_x,
                    goal_state.y-goal_closest_point_y,
                    goal_state.z-goal_closest_point_z,
                    yaw_error,
                    last_vel_mag,
                    last_commanded_world_velocity.yaw);
        fwrite(debug_msg, strlen(debug_msg), 1, fp);
        fclose(fp);
    }
#endif
...
int goto_waypoint_for_return(FlatVars input_state, FlatVars goal_state, FlatVars last_commanded_world_velocity, bool stop_at_waypoint, FlatVars * cmd_out, uint8_t * reached_goal)
{
  // This function computes the desired velocity to reach a goal state (location+yaw) from a starting state while respecting dynamics and obeying a maximum velocity and acceleration
  // This function requires as input the previously specified velocity to ensure continuity in velocity.
  // All coordinates are in the world frame. This means that body commanded input velocities will need to be transformed to the world frame and outputs that should be in the body frame will need to be transformed.
  ...
  *reached_goal = 0b11111111;
  ...
  // compute maximum possible velocity capable of stopping under maximum acceleration
  float max_vel_close_mag = sqrtf(2*max_lin_acceleration_for_return*goal_vec_mag);

  // now compute the desired magnitude of velocity as the smaller of the stopping velocity and max allowed velocity
  float des_velocity = max_lin_velocity_for_return;

  if(stop_at_waypoint == true)
  {
    MIN(des_velocity,max_vel_close_mag,max_lin_velocity_for_return);
  }
  ...
  // compute applied acceleration to influence velocity in correct direction
  float accel_applied_x = unit_des_vel_change_x*max_lin_acceleration_for_return;
  float accel_applied_y = unit_des_vel_change_y*max_lin_acceleration_for_return;
  float accel_applied_z = unit_des_vel_change_z*max_lin_acceleration_for_return;
  ...
  // bound current velocity by specified limit
  float current_vel_mag = sqrtf(current_cmd_world_velocity.x*current_cmd_world_velocity.x+current_cmd_world_velocity.y*current_cmd_world_velocity.y+current_cmd_world_velocity.z*current_cmd_world_velocity.z);
  if (current_vel_mag > max_lin_velocity_for_return)
  {
    current_cmd_world_velocity.x = current_cmd_world_velocity.x/current_vel_mag*max_lin_velocity_for_return;
    current_cmd_world_velocity.y = current_cmd_world_velocity.y/current_vel_mag*max_lin_velocity_for_return;
    current_cmd_world_velocity.z = current_cmd_world_velocity.z/current_vel_mag*max_lin_velocity_for_return;
  }
...
int goto_waypoint_with_gps(FlatVars input_state, FlatVars goal_state, FlatVars last_commanded_world_velocity, bool stop_at_waypoint, FlatVars * cmd_out, uint8_t * reached_goal)
{
  // This function computes the desired velocity to reach a goal state (location+yaw) from a starting state while respecting dynamics and obeying a maximum velocity and acceleration
  // This function requires as input the previously specified velocity to ensure continuity in velocity.
  // All coordinates are in the world frame. This means that body commanded input velocities will need to be transformed to the world frame and outputs that should be in the body frame will need to be transformed.
...
int goto_waypoint_for_circle(FlatVars input_state, FlatVars goal_state, FlatVars last_commanded_world_velocity, bool stop_at_waypoint, FlatVars * cmd_out, uint8_t * reached_goal, float goal_deviation)
{
  ...
  *reached_goal = 0b11111111;
  ...
  // if yaw error within threshhold of zero, set set yaw position flag to 0
  //if((yaw_error_abs<.008) || (stop_at_waypoint == false))
  if((yaw_error_abs<.015) || (stop_at_waypoint == false))
  //if((yaw_error_abs<.03) || (stop_at_waypoint == false))
  {
    *reached_goal=*reached_goal&0b11101111;
  }
  // if yaw velocity within threshhold of zero, set set yaw velocity flag to 0
...
int goto_waypoint_for_reduce_height(FlatVars input_state, FlatVars goal_state, FlatVars last_commanded_world_velocity, bool stop_at_waypoint, FlatVars * cmd_out, uint8_t * reached_goal)
{
  // This function computes the desired velocity to reach a goal state (location+yaw) from a starting state while respecting dynamics and obeying a maximum velocity and acceleration
  // This function requires as input the previously specified velocity to ensure continuity in velocity.
  // All coordinates are in the world frame. This means that body commanded input velocities will need to be transformed to the world frame and outputs that should be in the body frame will need to be transformed.
  ...
  *reached_goal = 0b11111111;
  ...
  // bound dt
  if (dt<.001){dt=.001;}
  else if (dt>.3){initialized = 0;dt = 0.1;}
  // save persistent time for next function call
  t_prev = t_now;
  ...
  // compute maximum possible velocity capable of stopping under maximum acceleration
  float max_vel_close_mag = sqrtf(2*max_lin_acceleration_reduce_height*goal_vec_mag);

  // now compute the desired magnitude of velocity as the smaller of the stopping velocity and max allowed velocity
  float des_velocity = max_lin_velocity_reduce_height;

  if(stop_at_waypoint == true)
  {
    MIN(des_velocity,max_vel_close_mag,max_lin_velocity_reduce_height);
  }
  ...
  // compute applied acceleration to influence velocity in correct direction
  float accel_applied_x = unit_des_vel_change_x*max_lin_acceleration_reduce_height;
  float accel_applied_y = unit_des_vel_change_y*max_lin_acceleration_reduce_height;
  float accel_applied_z = unit_des_vel_change_z*max_lin_acceleration_reduce_height;
  ...
  // bound current velocity by specified limit
  float current_vel_mag = sqrtf(current_cmd_world_velocity.x*current_cmd_world_velocity.x+current_cmd_world_velocity.y*current_cmd_world_velocity.y+current_cmd_world_velocity.z*current_cmd_world_velocity.z);
  if (current_vel_mag > max_lin_velocity_reduce_height)
  {
    current_cmd_world_velocity.x = current_cmd_world_velocity.x/current_vel_mag*max_lin_velocity_reduce_height;
    current_cmd_world_velocity.y = current_cmd_world_velocity.y/current_vel_mag*max_lin_velocity_reduce_height;
    current_cmd_world_velocity.z = current_cmd_world_velocity.z/current_vel_mag*max_lin_velocity_reduce_height;
  }
